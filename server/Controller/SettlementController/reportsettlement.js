...
module.exports.reportSettlement = async function (req, res) {
    const user = req.user;
    // const {id} = req.body;
    ...
    try{
        ...
        switch (value) {
            case 1: {
                ...
                let merchantSettlement = resultMerchantSettlement.map((item, index) => ({
        ...
        // Payout Currency
            case 2: {
                ...
                let PayoutCurrency = resultPayoutCurrency.map((item, index) => ({
                    'Sr': index + 1,
                ...
                },
                {
                    ...
                    'UTR Number': 'INR',
                    'Mode': '',
                    'Account Number': resultPayoutCurrency.filter((item) => item.currency === 'INR').reduce((total, current) => { return total += Number(current.amount)}, 0),
                    'IFSC Code': '',
                    'Bank Name': resultPayoutCurrency.filter((item) => item.currency === 'INR').reduce((total, current) => { return total += Number(current.akonto_charge)}, 0),
                    'Amount': '',
                    'Charges': '',
                    'GST Charges': '',
                    'Currency': '',
        ...
        // Settlement By Currency
            case 3: {
                ...
                let SettlementCurrency = resultSettlementCurrency.map((item, index) => ({
                    'Sr': index + 1,
                    'User Id': item.users_id,
                    'Merchant Name': item.merchant_name,
                ...
                for (x in currencies) {
                    ...
                    'Settlement Type': '',
                    'From Currency': resultSettlementCurrency.filter((item) => item.fromCurrency === currencies[x]).reduce((total, current) => { return total += Number(current.requestedAmount)}, 0),    
                    'To Currency': '',
                    'Exchange Rate': resultSettlementCurrency.filter((item) => item.fromCurrency === currencies[x]).reduce((total, current) => { return total += Number(current.charges)}, 0),
                    'Wallet Address': '',
                    'Account Number': resultSettlementCurrency.filter((item) => item.fromCurrency === currencies[x]).reduce((total, current) => { return total += Number(current.net_amount_for_settlement)}, 0),
                    'Bank Name': '',
                    'Branch Name': resultSettlementCurrency.filter((item) => item.fromCurrency === currencies[x]).reduce((total, current) => { return total += Number(current.settlementAmount)}, 0),
                    'City': '',  
                    'Zip Code': '',
                    'Country': '',
                    'Swiftcode': '',
        ...
        // Settlement From Bank/Acquirer
            case 4:{
                ...
                let SettlementFromBankAcquirer = resultSettlementFromBankAcquirer.map((item, index) => ({
                    'Sr': index + 1,
                    'Bank Name': item.bank_name,
                    'Amount': item.amount,
        ...
        // Commission Through International Settlement
            case 5:{
                ...
                let CommissionThroughInternationalSettlement = resultCommissionThroughInternationalSettlement.map((item, index) => ({
                    'Sr': index + 1,
                    'User Id': item.users_id,
                    'Merchant Name': item.merchant_name,
                    'Settlement Id': item.settlementId,
                ...
                },
                {
                    ...
                    'Merchant Name': '',
                    'Settlement Id': '',
                    'Settlement Type': resultCommissionThroughInternationalSettlement.filter((item) => item.requestedAmounts).reduce((total, current) => { return total += Number(current.requestedAmounts)}, 0),
                    'From Currency': '',    
                    'To Currency': resultCommissionThroughInternationalSettlement.filter((item) => item.charge).reduce((total, current) => { return total += Number(current.charge)}, 0),
                    'Exchange Rate': '',
                    'Wallet Address': resultCommissionThroughInternationalSettlement.filter((item) => item.NetSettlementAmount).reduce((total, current) => { return total += Number(current.NetSettlementAmount)}, 0),
                    'Account Number': '',
                    'Bank Name': resultCommissionThroughInternationalSettlement.filter((item) => item.settlementAmounts).reduce((total, current) => { return total += Number(current.settlementAmounts)}, 0),
                    'Branch Name': '',
                    'City': '',  
                    'Zip Code': '',
                    'Country': '',
        ...
        // Commission By Merchant
            case 6:{
                ...
                let CommissionByMerchantSettlement3 = resultCommissionByMerchantSettlement3.map((item, index) => ({
                    'Sr': index + 1, 
                    'User Id': item.user_id,
                    'Merchant Name': item.merchant_name,
                ...
                let CommissionByMerchantPayout2 = resultCommissionByMerchantPayout2.map((item, index) => ({
                    'Sr': index + 1, 
                    'User Id': item.users_id,
                ...
                let CommissionByMerchantDeposite1 = resultCommissionByMerchantDeposite1.map((item, index) => ({
                    'Sr': index + 1, 
                    'User Id': item.user_id,
                    'Merchant Name':item.name,
                    'Amount': item.Amount, 
        ...
        // Commission By Deposite    
            case 7:{
                ...
                let CommissionDeposite = resultCommissionDeposite.map((item, index) => ({
                    'Sr': index + 1,
                ...
                },
                {
                    ...
                    'Order No': '',
                    'Transaction': '',
                    'Card No': resultCommissionDeposite.filter((item) => item.ammount).reduce((total, current) => { return total += Number(current.ammount)}, 0),
                    'Amount': '',
                    'Paying Charge': resultCommissionDeposite.filter((item) => item.payin_charges).reduce((total, current) => { return total += Number(current.payin_charges)}, 0),    
                    'Gst': '',
                    'Currency': '',
                    'Method': '',
                    'Country': '',
        ...
        // Commission By Payout
            case 8:{
                ...
                let CommissionPayout = resultCommissionPayout.map((item, index) => ({
                    'Sr': index + 1,
                ...
                },
                {
                    ...
                    'Transaction No': '',
                    'UTR No': '',
                    'Method': resultCommissionPayout.filter((item) => item.amount).reduce((total, current) => { return total += Number(current.amount)}, 0),
                    'Amount': '',
                    'Payout Charge': resultCommissionPayout.filter((item) => item.akonto_charge).reduce((total, current) => { return total += Number(current.akonto_charge)}, 0),
                    'GST': '',
                    'Status': '',
                    'Currency': '',
                    'Created_on': '',
        ...
        // Commission through ChargeBack/Disputes
            case 10:{
                ...
                let CommissionChargeback = resultCommissionChargeback.map((item, index) => ({
                    'Sr': index + 1,
                    'Order No': item.order_no,
                    'Transaction': item.transaction_id,
                    'Card No': item.card_4_4,
                ...
                },
                {
                    ...
                    'Transaction': '',
                    'Card No': '',
                    'Amount': resultCommissionChargeback.filter((item) => item.ammount).reduce((total, current) => { return total += Number(current.ammount)}, 0),
                    'Paying Charge': '',    
                    'Gst': resultCommissionChargeback.filter((item) => item.payin_charges).reduce((total, current) => { return total += Number(current.payin_charges)}, 0),
                    'Currency': '',
                    'Method': '',
                    'Country': '',
                    'Status': '',
        ...
        // Chargeback/Disputes Merchants Wise
            case 11:{
                ...
                let ChargebackDisputesMerchantsWise = resultChargebackDisputesMerchantsWise.map((item, index) => ({
                    'Sr': index + 1, 
                    'User Id': item.user_id,
                    'Amount': item.ammount, 
                    'Paying Charge': item.payin_charges,    
