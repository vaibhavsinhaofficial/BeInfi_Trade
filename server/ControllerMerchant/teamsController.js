...
module.exports.default = async function(req,res){
    try {
        ...
        if(id) {
            ...
            let sql1 = "SELECT tbl_user.*, DATE_FORMAT(created_on, '%Y-%m-%d %H:%i:%s') AS created_on, DATE_FORMAT(updated_on, '%Y-%m-%d %H:%i:%s') AS updated_on FROM tbl_user WHERE parent_id = ? AND account_type = 3 ORDER BY created_on DESC limit ?,?";

            let result1 =  await mysqlcon(sql1,[id, start, limit]);
            ...
            let result1 =  await mysqlcon(sql1,[user.id,start, limit]);
            let startRange = start + 1;
            let endRange = start + result1.length;
...
module.exports.createEmployee = async function(req,res){
    try {
        ...
        if(id) {
            ...
            let sql = "SELECT email FROM tbl_user WHERE parent_id = ? && email = ? && account_type = 3"
            // checking if email exist or not
            let result =  await mysqlcon(sql,[user.id,email]);
            if(result.length === 0){
                let sql1 = "SELECT mobile_no FROM tbl_user WHERE parent_id = ? && mobile_no = ? && account_type = 3";
                // checking if mobile number exist or not
                let result1 =  await mysqlcon(sql1,[id,mobile_no]);
                ...
                if(result1.length === 0){
                        if(email && emailvalidator.validate(email)){``
                            ...
                            if(!result2){
                                ...
                                send_mail.mail({email: email,mobile_no: mobile_no,name : name, usercode: usercode,password : defaultPassword, subject:"Team Create"}, 'employee');
                                return res.json(200,{
...
module.exports.getTeamDetails = async function(req, res){
    try {
        let {id} = req.body
...
module.exports.teamEditDetails = async function (req, res) {
    try {
...
module.exports.verifyTeam = async function (req, res) {
    try {
        let {id} = req.body;
