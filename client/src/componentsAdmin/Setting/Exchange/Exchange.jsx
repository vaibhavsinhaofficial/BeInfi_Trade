...
function Exchange({authCreate, authRead, authUpdate, authDelete}) {
  const tableHeading = [
    "Exchange title",
    "Rate",
    "Created On",
  ...
  const deleteRow = async (id) => {
    try {
      ...
      if (answer) {
        ...
        let result = await axios.post(`${baseUrl}/deleteExchange`, formData, config);
            ...
            tableData.map((item, index) => {
              return (
                <TableRow
                  ...
                  key={index}
                >
                  <TableCell align="center">{item.exchange_title}</TableCell>
                  <TableCell align="center">{item.rate}</TableCell>
                  <TableCell align="center">{item.created_on}</TableCell>
                  <TableCell align="center">
                  {authUpdate? 
                    <DialogOpenModelEdit ID={item.id} authUpdate={authUpdate} ReadData={ReadData}/>
                  : null}
    ...
    <>
      <Box component="main" className="allcol restBody">
        <div className="row align-items-center">
          <h4 className="mb-3 headingAll">Exchange Rate List</h4>
...
const DialogOpenModelAdd = ({ authCreate, ReadData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let result = await axios.post(`${baseUrl}/createExchange`, values, config);
      ReadData()
      handleClose()
                    ...
                    <div className="col-12 swapBox">
                      <form className="row justify-content-around"  onSubmit={handleSubmit}>
                        <div className="row">
                          ...
                          <div className="col-8">
                            <input type="text" placeholder="Exchange title" className="form-control mb-3" onChange={(e) => setExchange_title(e.target.value)} />
                          </div>
                        </div>
                        <div className="row">
                          <div className="col-4">
...
const DialogOpenModelEdit = ({ authUpdate, ID, ReadData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let result = await axios.post(`${baseUrl}/updateExchange`, values, config);
      ReadData()
      handleClose()
    } catch (error) {
      console.log(error);
                  ...
                  <div className="col-12 swapBox">
                    <form className="row justify-content-around"  onSubmit={handleSubmit}>
                      <div className="row">
                        ...
                        <div className="col-8">
                          <input type="text" placeholder="Exchange title" className="form-control mb-3" value={exchange_title} onChange={(e) => setExchange_title(e.target.value)} />
                        </div>
                      </div>
                      <div className="row">
                        <div className="col-4">
