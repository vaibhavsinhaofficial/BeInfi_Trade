...
function BankDeposit({authCreate, authRead, authUpdate, authDelete}) {
  ...
  const bankDepositsCard = async()=>{
    ...
    axios
      .post(`${baseUrl}/bankDepositsCards`, values, config)
      .then((res) => {
        // console.log(res);
  ...
  const downloadExl = () => {
    ...
    axios.post(`${baseUrl}/bankDepositDownload`, values, config)
    .then((res) => {
      const workSheet = XLSX.utils.json_to_sheet(res.data);
      const workBook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workBook, workSheet, "BankDeposit");
      // Buffer
...
const tableHeading = ['Merchant Name','Transaction id','Received Date','Currency','Bank Name','Transaction Type','Deposits Received','Bank Charges','Tax','Total Charges','Net Deposits Received','Authorizer','Action']
  ...
  return (
    ...
    <div className="row align-items-center justify-content-end mt-3 mb-3">
     ...
     {/* FILTER SECTION END*/}
    <TableComp setXlData={setXlData} tableHeading={tableHeading} tableBodyData={tableBodyData} fetchData={fetchData} loading={loading} authUpdate = {authUpdate}/>
    <PaginationComp
          setPage={setPage}
          page={page}
          totalPage={totalPage}
