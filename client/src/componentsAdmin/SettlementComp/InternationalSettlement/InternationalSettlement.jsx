...
import EditInternationalSettlement from "./EditInternationalSettlement";
import NewInternationalSettlement from "./NewInternationalSettlement";

function InternationalSettlement({authCreate, authRead, authUpdate, authDelete}) {
  const tableHeading = ['Final Authorization','Admin Status','Requested Date','Merchant Name','Source','Settlement Id','Settlement Type','Bank Name','From Currency', 'To Currency', 'Exchange Rate','Amount ','Settlement Fee','Net Amount For Settlement','Settlement Amount	','Action'];
  ...
  const ReadData = async () => {
    try {
      ...
      let result = await axios.post(
        `${baseUrl}/api/settelment/internationalSettlement`,
        values,
        config
      );
  ...
  const internationalCards = async () =>{
    const auth = localStorage.getItem("admin");
    ...
    axios
      .post(`${baseUrl}/api/settelment/internationalCards`, values, config)
      .then((res) => {
        // console.log(res);
  ...
  const downloadExl = () => {
    ...
    axios.post(`${baseUrl}/defaultInternationalDownload`, values, config)
    .then((res) => {
      const workSheet = XLSX.utils.json_to_sheet(res.data);
      const workBook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workBook, workSheet, "InternationalSettlement");
      // Buffer
      let buf = XLSX.write(workBook, { bookType: "xlsx", type: "buffer" });
      // Binary String
      XLSX.write(workBook, { bookType: "xlsx", type: "binary" });
      // Download
      XLSX.writeFile(workBook, "InternationalSettlement.xlsx");
    })
    .catch((err) => console.log(err));
  }
    ...
    <>
      <Box component="main" className="allcol restBody">
        <div className="row align-items-center mt-3 mb-3">
          <h4 className="mb-3 headingAll">International Settlement List</h4>
          ...
          <div className="col-3 mt-3 text-end">
            {
              authCreate ? (
                <NewInternationalSettlement groupid={groupid} />
              ) : null
            }
          </div>
        </div>
...
const PopUps = ({item, authUpdate, authRead, groupid }) => {
    ...
    <>
      <div>
        ...
        <div className="d-flex  align-items-center justify-content-around flex-column">
          {
            authRead ? (
              <ViewInternational item={item} groupid={groupid}/>
            ) : null
          }
          {
            authUpdate ? (
              <EditInternationalSettlement ID={item.id} groupid={groupid} />
            ) : null
          }
        </div>
