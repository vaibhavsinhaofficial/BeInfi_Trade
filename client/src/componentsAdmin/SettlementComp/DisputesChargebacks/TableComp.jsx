...
export default function TableComp({ tableBodyData,tableHeading, authRead, loading, fetchData }) {
  if(loading){
    return <Loader />
  }
  return (
            ...
            {Object.keys(tableBodyData).length > 0 ? (
              ...
              return (
                ...
                >
                  ...
                  <TableCell style={{whiteSpace: "nowrap"}}>{item.mode}</TableCell>
                  <TableCell>{item.auth}</TableCell>
                  <TableCell>{item.net_amount}</TableCell>
                  <TableCell align="center">
                    {
                      authRead ? (
                        <PopUp formData={item} ID={item.transaction_id} />
                      ) : null
                    }
                  </TableCell>
                </TableRow>
...
const DialogOpenModel1 = ({ ID }) => {
  ...
  const details = async () => {
    try {
      ...
      setType(result.data.result.mode)
      setTransaction_id(result.data.result.transaction_id)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let result = await axios.post(`${baseUrl}/updateDisputes`, values, config);
      ...
      <div>
        ...
        >
          <DialogTitle
            ...
            style={{ fontWeight: "700", fontSize: "20px" }}
          >
            Edit Disputes/Chargebacks Transaction
          </DialogTitle>
          <DialogContent>
            <div className="row">
              <div className="col-12 dialogBlock1">
